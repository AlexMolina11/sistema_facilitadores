    ...
    <aside class="cv-config-panel">
        <div class="cv-config-sticky">
            ...
            <div class="accordion" id="cvAccordion">
                ...
                @include('fac.cv.partials.config-list', [
                    'titulo' => 'Referencias',
                    'collapseId' => 'cvReferencias',
                    'section' => 'referencias',
                    'items' => $cvData['referencias'],
                    'fields' => [
                        'tipo' => 'Tipo',
                        'nombre' => 'Nombre',
                        'telefono' => 'Teléfono',
                ...
                <div class="accordion-item">
                    <h2 class="accordion-header">
                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#cvAreas">
                            Áreas y habilidades técnicas
                        </button>
                    </h2>
                    <div id="cvAreas" class="accordion-collapse collapse" data-bs-parent="#cvAccordion">
                        <div class="accordion-body">
                            @forelse($cvData['areas'] as $area)
                                <div class="cv-config-item">
                                    <div class="form-check cv-check fw-bold">
                                        <input class="form-check-input cv-toggle" type="checkbox" checked
                                            data-section="areas" data-item="{{ $area['id'] }}" data-field="nombre"
                                            id="area_{{ $area['id'] }}">
                                    ...
                                    @foreach($area['habilidades'] as $hab)
                                        <div class="form-check cv-check ms-3">
                                            <input class="form-check-input cv-toggle" type="checkbox" checked
                                                data-section="habilidades_area_{{ $area['id'] }}" data-item="{{ $hab['id'] }}" data-field="nombre"
                                                id="hab_{{ $area['id'] }}_{{ $hab['id'] }}">
                                            <label class="form-check-label" for="hab_{{ $area['id'] }}_{{ $hab['id'] }}">
                                                {{ $hab['nombre'] }}
                                            </label>
                                        </div>
                                    @endforeach
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderMineducyt() {
        ...
        html += `
            <table class="cv-table">
                ...
                <tbody>${renderRows('idiomas', data.idiomas, ['idioma', 'nivel', 'certificado']) || '<tr><td colspan="3">&nbsp;</td></tr>'}</tbody>
            </table>
        `;
    ...
    function renderProfesional() {
        ...
        let html = `
            <div class="cv-pro">
                <aside class="cv-pro-sidebar">
                    <div class="cv-pro-photo">
                        ${data.personal.foto 
                            ? `<img src="${escapeHtml(data.personal.foto)}" alt="Foto">` 
                            : `<div class="cv-pro-photo-empty">${personal('nombre') ? personal('nombre').substring(0, 2).toUpperCase() : 'CV'}</div>`
                        }
                    </div>
                    ...
                    <ul>
                        ${data.areas.filter(a => isSelected('areas', a.id, 'nombre')).map(a => `<li>${escapeHtml(a.nombre)}</li>`).join('') || '<li>Sin áreas registradas</li>'}
                    </ul>
                    ...
                    <ul>
                        ${data.disponibilidades.filter(item => rowIsVisible('disponibilidades', item, ['nombre'])).map(item => `<li>${escapeHtml(item.nombre)}</li>`).join('') || '<li>No registrada</li>'}
                    </ul>
                </aside>
                ...
                <main class="cv-pro-main">
                    <div class="cv-pro-header">
                        ...
                        <p>Consultor/a profesional FEPADE</p>
                        <div>
                            ${personal('nacionalidad') ? `<span>${personal('nacionalidad')}</span>` : ''}
                            ${personal('residencia') ? `<span>${personal('residencia')}</span>` : ''}
                            ${personal('fecha_nacimiento') ? `<span>${personal('fecha_nacimiento')}</span>` : ''}
                        </div>
                    </div>

                    <section>
                        <h2>Experiencia profesional</h2>
                        ${data.experiencias.filter(item => rowIsVisible('experiencias', item, ['empresa','cargo','descripcion','desde','hasta'])).map(item => `
                            <div class="cv-pro-item">
                                <div class="cv-pro-item-head">
                                    <strong>${isSelected('experiencias', item.id, 'cargo') ? escapeHtml(item.cargo) : ''}</strong>
                                    <span>${isSelected('experiencias', item.id, 'desde') ? escapeHtml(item.desde) : ''} - ${isSelected('experiencias', item.id, 'hasta') ? escapeHtml(item.hasta) : ''}</span>
                                </div>
                                ${isSelected('experiencias', item.id, 'empresa') ? `<p class="cv-pro-place">${escapeHtml(item.empresa)}</p>` : ''}
                                ${isSelected('experiencias', item.id, 'descripcion') ? `<p>${escapeHtml(item.descripcion)}</p>` : ''}
                            </div>
                        `).join('') || '<p>Sin experiencia registrada.</p>'}
                    </section>

                    <section>
                        <h2>Formación, atestados y consultorías</h2>
                        ${data.atestados.filter(item => rowIsVisible('atestados', item, ['titulo','institucion','tipo_formacion','fecha_fin','descripcion'])).map(item => `
                            <div class="cv-pro-item">
                                <div class="cv-pro-item-head">
                                    <strong>${isSelected('atestados', item.id, 'titulo') ? escapeHtml(item.titulo) : ''}</strong>
                                    <span>${isSelected('atestados', item.id, 'fecha_fin') ? escapeHtml(item.fecha_fin) : ''}</span>
                                </div>
                    ...
                    <section>
                        <h2>Capacitaciones FEPADE</h2>
                        ${data.capacitaciones_fepade.filter(item => rowIsVisible('capacitaciones_fepade', item, ['nombre_evento','tema','institucion','fecha_inicio','fecha_fin','horas'])).map(item => `
                            <div class="cv-pro-item">
                                <div class="cv-pro-item-head">
                                    <strong>${isSelected('capacitaciones_fepade', item.id, 'nombre_evento') ? escapeHtml(item.nombre_evento) : ''}</strong>
                                    <span>${isSelected('capacitaciones_fepade', item.id, 'fecha_inicio') ? escapeHtml(item.fecha_inicio) : ''} - ${isSelected('capacitaciones_fepade', item.id, 'fecha_fin') ? escapeHtml(item.fecha_fin) : ''}</span>
                                </div>
                                ${isSelected('capacitaciones_fepade', item.id, 'institucion') ? `<p class="cv-pro-place">${escapeHtml(item.institucion)}</p>` : ''}
                                ${isSelected('capacitaciones_fepade', item.id, 'tema') ? `<p>${escapeHtml(item.tema)}</p>` : ''}
                            </div>
                        `).join('') || '<p>Sin capacitaciones FEPADE registradas.</p>'}
                    </section>

                    <section>
                        <h2>Referencias</h2>
                        <div class="cv-pro-grid">
                            ${data.referencias.filter(item => rowIsVisible('referencias', item, ['nombre','telefono','correo','empresa','cargo'])).map(item => `
                                <div class="cv-pro-ref">
                                    <strong>${isSelected('referencias', item.id, 'nombre') ? escapeHtml(item.nombre) : ''}</strong>
                                    ${isSelected('referencias', item.id, 'cargo') ? `<span>${escapeHtml(item.cargo)}</span>` : ''}
                                    ${isSelected('referencias', item.id, 'empresa') ? `<span>${escapeHtml(item.empresa)}</span>` : ''}
                                    ${isSelected('referencias', item.id, 'telefono') ? `<span>${escapeHtml(item.telefono)}</span>` : ''}
                                    ${isSelected('referencias', item.id, 'correo') ? `<span>${escapeHtml(item.correo)}</span>` : ''}
                                </div>
                            `).join('') || '<p>Sin referencias registradas.</p>'}
                        </div>
                    </section>
                </main>
            </div>
